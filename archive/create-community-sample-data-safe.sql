-- Create Sample Community Data for BA Property Manager (Safe Version)
-- This script populates the community tables with realistic sample data
-- Safe to run multiple times - will not create duplicate data
...
DECLARE
    ...
    post_count INTEGER;
BEGIN
    -- Check if we already have community groups
    SELECT COUNT(*) INTO group_count FROM community_groups;
    
    IF group_count > 0 THEN
        RAISE NOTICE 'Community groups already exist (% groups found). Skipping sample data creation.', group_count;
        RETURN;
    END IF;
    
    RAISE NOTICE 'No existing community groups found. Creating sample data...';
END $$;

-- Only proceed if we don't have existing data
DO $$
DECLARE
    group_count INTEGER;
BEGIN
    ...
    IF group_count = 0 THEN
        ...
        RAISE NOTICE 'Sample community groups created successfully!';
    END IF;
END $$;

-- Now let's add some members to these groups (only if groups exist and have no members)
DO $$
DECLARE
    group_ids UUID[];
    user_ids UUID[];
...
BEGIN
    ...
    IF member_count > 0 THEN
        RAISE NOTICE 'Group members already exist (% members found). Skipping member creation.', member_count;
        RETURN;
    END IF;
    ...
    RAISE NOTICE 'Sample group members created successfully!';
END $$;

-- Now let's add some sample posts to the groups (only if no posts exist)
DO $$
DECLARE
    post_count INTEGER;
BEGIN
    -- Check if we already have posts
    SELECT COUNT(*) INTO post_count FROM group_posts;
    
    IF post_count > 0 THEN
        RAISE NOTICE 'Group posts already exist (% posts found). Skipping post creation.', post_count;
        RETURN;
    END IF;
    
    -- Add sample posts to the groups
    ...
    RAISE NOTICE 'Sample group posts created successfully!';
END $$;
...
SELECT 'Community Sample Data Status:' as info;
SELECT 
    CASE 
        WHEN COUNT(*) > 0 THEN '✅ Sample data created successfully'
        ELSE 'ℹ️ Sample data already exists or no data created'
    END as status,
